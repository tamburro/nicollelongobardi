...
/* ─── Skills com posições distribuídas no topo ───────────── */
const skills: { label: string; coral?: boolean }[] = [
  { label: 'Produção executiva',       coral: true },
  { label: 'Gestão de projetos',       coral: true },
  { label: 'Planejamento estratégico', coral: true },
  { label: 'Liderança de equipe',      coral: true },
  { label: 'Artes cênicas',            coral: true },
  { label: 'Produção de eventos',      coral: true },
...
function skillPos(i: number, _total?: number): { x: string; y: string } {
  const cols = 6
  const col  = i % cols
  const row  = Math.floor(i / cols)
  const xBase   = 8 + (col / (cols - 1)) * 84
  const xJitter = Math.sin(i * 6.1 + 1.3) * 6
  const yBase   = 4 + row * 15
  return {
    x: `${Math.min(92, Math.max(8, xBase + xJitter)).toFixed(1)}%`,
    y: `${Math.min(55, yBase).toFixed(1)}%`,
  }
}
...
function Eyebrow({ children }: { children: string }) {
  return (
    <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.1em] text-[#e8573c] mb-3">
      <span className="block w-5 h-0.5 rounded bg-[#e8573c]" />
      {children}
    </p>
  )
}
...
function Tag({ children }: { children: string }) {
  return (
    <span className="text-[13px] font-semibold text-[#222222] bg-[#f7f7f7] border border-[#dddddd] px-3.5 py-1.5 rounded-full transition-colors hover:bg-[rgba(232,87,60,0.10)] hover:text-[#e8573c] hover:border-[rgba(232,87,60,0.2)] cursor-default select-none">
      {children}
    </span>
  )
}
...
function ExpCard({ exp }: { exp: Experience }) {
  return (
    <div className="bg-white border border-[#dddddd] rounded-2xl p-7 shadow-[rgba(0,0,0,0.04)_0_2px_8px_0] transition-[box-shadow,border-color] hover:shadow-[rgba(0,0,0,0.08)_0_4px_16px_0] hover:border-[#c1c1c1]">
      <div className="flex items-start gap-4 mb-5">
        <div className="w-12 h-12 rounded-[10px] bg-[#f7f7f7] border border-[#dddddd] flex items-center justify-center shrink-0 font-display text-[13px] font-bold text-[#6a6a6a]">
          {exp.logo}
        </div>
        <div>
          ...
          <p className="text-[13px] font-medium text-[#c1c1c1]">{exp.period}</p>
        </div>
      </div>
      {exp.bullets && exp.bullets.length > 0 && (
        <ul className="flex flex-col gap-1.5">
          {exp.bullets.map((b, i) => (
            <li key={i} className="text-[15px] font-medium text-[#222222] leading-[1.55] pl-4 relative before:absolute before:left-0 before:top-[9px] before:w-[5px] before:h-[5px] before:rounded-full before:bg-[#e8573c]">
              {b}
            </li>
          ))}
        </ul>
      ...
      {exp.multiRoles && (
        <div className="flex flex-col gap-5">
          {exp.multiRoles.map((r, i) => (
            <div key={i} className="border-l-2 border-[#dddddd] pl-4">
              <p className="text-[14px] font-semibold text-[#222222] mb-1">{r.title}</p>
              <p className="text-[13px] font-medium text-[#c1c1c1] mb-2">{r.period}</p>
              {r.bullets.length > 0 && (
                <ul className="flex flex-col gap-1.5">
                  {r.bullets.map((b, j) => (
                    <li key={j} className="text-[15px] font-medium text-[#222222] leading-[1.55] pl-4 relative before:absolute before:left-0 before:top-[9px] before:w-[5px] before:h-[5px] before:rounded-full before:bg-[#e8573c]">
                      {b}
                    </li>
                  ))}
                </ul>
      ...
      {exp.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-4 pt-4 border-t border-[#dddddd]">
...
export default function Home() {
  ...
  return (
    <>
      {/* ── NAV ──────────────────────────────────────────── */}
      <nav className="fixed inset-x-0 top-0 z-50 h-[72px] bg-white/90 backdrop-blur-md border-b border-[#dddddd]">
        <div className="max-w-[1200px] mx-auto px-8 h-full flex items-center justify-between">
          ...
          <ul className="hidden md:flex items-center gap-7 list-none">
            {navLinks.map(l => (
              <li key={l.href}>
                <a href={l.href} className="text-[14px] font-medium text-[#222222] no-underline hover:text-[#e8573c] transition-colors">
                  {l.label}
                </a>
              </li>
            ))}
          </ul>
          <a href="#contato" className="hidden md:inline-flex items-center text-[14px] font-medium text-white bg-[#e8573c] rounded-full px-5 py-2.5 no-underline hover:bg-[#c94028] transition-colors">
            Contato
          </a>
      ...
      <div className={`fixed top-0 right-0 bottom-0 w-[280px] bg-white border-l border-[#dddddd] z-[200] flex flex-col gap-7 px-8 pt-[88px] pb-8 transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {navLinks.map(l => (
          <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="text-lg font-medium text-[#222222] no-underline">{l.label}</a>
        ))}
        <a href="#contato" onClick={() => setMenuOpen(false)} className="mt-2 flex justify-center text-[15px] font-medium text-white bg-[#e8573c] rounded-full px-5 py-3 no-underline">Contato</a>
      </div>

      {/* ── HERO ─────────────────────────────────────────── */}
      <section id="hero" className="min-h-screen flex items-center pt-[120px] pb-20 px-8 bg-white">
        <div className="max-w-[1200px] w-full mx-auto grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-16 lg:gap-20 items-center">
          <div>
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.1em] text-[#e8573c] mb-5">
              <span className="block w-6 h-0.5 rounded bg-[#e8573c]" />
            ...
            <p className="flex items-center gap-1.5 text-sm font-medium text-[#c1c1c1] mb-11 before:content-[''] before:w-1.5 before:h-1.5 before:rounded-full before:bg-[#e8573c]">
              Rio de Janeiro, Brasil
            </p>
            <div className="flex gap-3 flex-wrap">
              <a href="#contato" className="inline-flex items-center gap-2 text-base font-medium text-white bg-[#e8573c] rounded-full px-7 py-3.5 no-underline hover:bg-[#c94028] hover:-translate-y-px transition-all active:scale-95">
              ...
              <a href="#experiencia" className="inline-flex items-center text-base font-medium text-[#222222] bg-white border border-[#dddddd] rounded-full px-7 py-3.5 no-underline hover:border-[#222222] hover:-translate-y-px transition-all">
                Ver experiência
              </a>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-5">
                <p className="text-lg font-medium text-[#222222] leading-[1.60]">
                  Com formação em <strong className="text-[#e8573c] font-semibold">Artes Cênicas</strong>, sou educadora por vocação, atriz por paixão e produtora por vontade de realização. Minha trajetória atravessa o palco, a sala de aula e os bastidores dos grandes eventos culturais do Rio de Janeiro.
                </p>
                <p className="text-lg font-medium text-[#222222] leading-[1.60]">
                  Tenho experiências em <strong className="text-[#e8573c] font-semibold">Gestão de Projetos</strong>, Planejamento Estratégico e Produção de Eventos — de eventos com 3.000 pessoas a programas educacionais que alcançaram mais de 25.000 crianças em escolas públicas de todo o estado.
                </p>
                <p className="text-lg font-medium text-[#222222] leading-[1.60]">
                  Atualmente elaboro e concebo projetos para <strong className="text-[#e8573c] font-semibold">Editais Públicos e Leis de Incentivo à Cultura e ao Esporte</strong>.
                </p>
              </div>
              <a href="#contato" className="mt-8 inline-flex items-center text-base font-medium text-white bg-[#e8573c] rounded-full px-7 py-3.5 no-underline hover:bg-[#c94028] transition-colors">
                Vamos conversar
              </a>
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {/* ── ÁREAS ────────────────────────────────────────── */}
      <section id="areas" className="py-24 px-8 bg-[#f7f7f7]">
        <div className="max-w-[1200px] mx-auto">
          <Eyebrow>Áreas de Atuação</Eyebrow>
          <SectionTitle>O que eu faço</SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {areas.map(a => (
              <div key={a.title} className="bg-white border border-[#dddddd] rounded-2xl p-8 transition-[box-shadow,border-color,transform] hover:shadow-[rgba(0,0,0,0.08)_0_4px_16px_0] hover:border-[#c1c1c1] hover:-translate-y-0.5 duration-200">
      ...
      {/* ── PROJETOS ─────────────────────────────────────── */}
      <section id="projetos" className="py-24 px-8 bg-[#f7f7f7]">
        <div className="max-w-[1200px] mx-auto">
          <Eyebrow>Portfólio</Eyebrow>
          <SectionTitle>Projetos em destaque</SectionTitle>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {projects.map(p => (
              <div key={p.title} className="bg-white rounded-[20px] overflow-hidden border border-[#dddddd] transition-[box-shadow,transform] hover:shadow-[rgba(0,0,0,0.08)_0_8px_24px_0] hover:-translate-y-1 duration-200">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="p-6">
                  <p className="text-[12px] font-semibold text-[#e8573c] uppercase tracking-[0.06em] mb-2">{p.tag}</p>
                  ...
                  {p.stats.length > 0 && (
                    <div className="flex gap-5 mt-5 pt-5 border-t border-[#dddddd]">
                      {p.stats.map((s, i) => (
                        <div key={i}>
                          <p className="font-display text-[22px] font-medium text-[#e8573c] tracking-[-0.5px] leading-none">{s.v}</p>
      ...
      <section id="competencias" className="pt-24 pb-0 bg-white overflow-hidden">
        <div className="max-w-[1200px] mx-auto px-8">
          <Eyebrow>Competências</Eyebrow>
          <SectionTitle className="mb-2">Habilidades</SectionTitle>
          <p className="text-sm font-medium text-[#6a6a6a] mb-4">
            Arraste e interaja com as habilidades.
          </p>
        </div>
          ...
          {skills.map((s, i) => {
            ...
            return (
              <MatterBody
                ...
                x={pos.x}
                y={pos.y}
                matterBodyOptions={{ friction: 0.4, restitution: 0.3, frictionAir: 0.011 }}
              >
                <div className={`px-4 py-2.5 rounded-full text-[13px] font-semibold whitespace-nowrap border cursor-grab active:cursor-grabbing select-none transition-colors ${s.coral ? 'bg-[#e8573c] text-white border-[#c94028] hover:bg-[#c94028]' : 'bg-[#f7f7f7] text-[#222222] border-[#dddddd] hover:bg-[rgba(232,87,60,0.10)] hover:text-[#e8573c] hover:border-[rgba(232,87,60,0.2)]'}`}>
                  {s.label}
                </div>
              </MatterBody>
            )
      ...
      <section className="py-24 px-8 bg-[#f7f7f7]">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
          <div>
            ...
            <div className="bg-white border border-[#dddddd] rounded-2xl p-6 hover:shadow-[rgba(0,0,0,0.04)_0_2px_8px_0] transition-shadow">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <div key={c.title} className="bg-white border border-[#dddddd] rounded-2xl p-5 hover:shadow-[rgba(0,0,0,0.04)_0_2px_8px_0] transition-shadow">
      ...
      <section id="contato" className="py-24 px-8 bg-white">
        <div className="max-w-[640px] mx-auto">
          <div className="bg-white rounded-[20px] border border-[#dddddd] shadow-[rgba(0,0,0,0.02)_0_0_0_1px,rgba(0,0,0,0.04)_0_2px_6px_0,rgba(0,0,0,0.10)_0_4px_8px_0] px-10 sm:px-14 py-14 text-center">
            ...
            <div className="flex flex-col gap-3.5 mb-9">
              {[
                ...
                <div key={i} className="flex items-center justify-center gap-2.5 text-base font-medium text-[#222222]">
                  {c.icon === 'phone'
                    ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#e8573c" strokeWidth="2" className="shrink-0"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 8.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg>
                    : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#e8573c" strokeWidth="2" className="shrink-0"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>
                  }
                  <a href={c.href} className="text-[#428bff] font-semibold no-underline hover:underline">{c.label}</a>
                </div>
              ))}
            </div>
            <hr className="border-[#dddddd] mb-9" />
            <a href="mailto:longobardiproduz@gmail.com" className="flex w-full items-center justify-center gap-2 text-base font-medium text-white bg-[#e8573c] rounded-full py-4 no-underline hover:bg-[#c94028] transition-colors">
      ...
      {/* ── FOOTER ───────────────────────────────────────── */}
      <footer className="bg-[#222222] py-10 px-8">
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-5">
            {navLinks.map(l => (
              <a key={l.href} href={l.href} className="text-[13px] font-medium text-[#c1c1c1] no-underline hover:text-white transition-colors">{l.label}</a>
            ))}
          </div>
          <span className="text-[13px] font-medium text-[#c1c1c1]">© 2025 · Rio de Janeiro, Brasil</span>
        </div>
      </footer>
    </>
  )
